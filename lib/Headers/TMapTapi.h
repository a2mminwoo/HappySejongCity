 ...
 *
 *  @abstract
 *      setBizAppID: 성공시 발생하는 callback
 */
- (void)bizAppIdSucceed;

/*
 *  bizAppIdFailed:
 *
 *  @abstract
 *      setBizAppID: 실패시 발생하는 callback
 */
- (void)bizAppIdFailed:(NSError*)error;

@end

@interface TMapTapi : NSObject

/** TMap 연동 API 사용인증 (apikey, bizAppId)
 * @param delegate 성공여부 콜백 Delegate
 * @param apiKey apiKey
 * @param bizAppId bizAppId
 */
+ (void)setSKPMapAuthenticationWithDelegate:(id<TMapTapiDelegate>)delegate apiKey:(NSString*)apiKey bizAppId:(NSString*)bizAppId;
